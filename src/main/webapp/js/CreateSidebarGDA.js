...
function createSidebarGDA(point, name, StateNM, Site_no, USGS_URL, base_url)
{
...
//    GEvent.addListener(marker, "click", function() {
        ...
        $.get(sos_url, function(xml) {
            $(xml).find('[nodeName="wml2:TimeseriesObservation"],TimeseriesObservation').each(function(){
                var watershed = $(this).find('[nodeName="om:value"],value').text();
                var name = $(this).find('[nodeName="gml:name"],name').text();
                var USGS_link = $(this).find('[nodeName="sf:sampledFeature"],sampledFeature').attr("xlink:href");
                ...
                USGS_link = '<a href = "' + USGS_URL + '" >' + Site_no + '</a>';
...
//                marker.openInfoWindowTabsHtml([new GInfoWindowTab(label1,(html + table_2))]);
                html = html + table_2
                return html;
            });
        });
